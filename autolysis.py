    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # If both encodings fail, try with error handling
                data = pd.read_csv(csv_file, encoding='latin-1', on_bad_lines='skip')
                print(f"Warning: Some lines were skipped due to encoding issues in {csv_file}")
    except Exception as e:
        print(f"Error reading {csv_file}: {e}")
        return None, None

    if data.empty:
        print(f"Warning: Empty dataset in {csv_file}")
        return None, None
